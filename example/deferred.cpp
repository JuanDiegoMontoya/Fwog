...
{
  auto vertexShader =
      Fwog::Shader(Fwog::PipelineStage::VERTEX_SHADER, Utility::LoadFile("shaders/SceneDeferred.vert.glsl"));
  auto fragmentShader =
      Fwog::Shader(Fwog::PipelineStage::FRAGMENT_SHADER, Utility::LoadFile("shaders/SceneDeferred.frag.glsl"));
...
Fwog::GraphicsPipeline CreateShadowPipeline()
{
  auto vertexShader =
      Fwog::Shader(Fwog::PipelineStage::VERTEX_SHADER, Utility::LoadFile("shaders/SceneDeferred.vert.glsl"));
  auto fragmentShader =
      Fwog::Shader(Fwog::PipelineStage::FRAGMENT_SHADER, Utility::LoadFile("shaders/RSMScene.frag.glsl"));
...
Fwog::GraphicsPipeline CreateShadingPipeline()
{
  ...
  auto fragmentShader =
      Fwog::Shader(Fwog::PipelineStage::FRAGMENT_SHADER, Utility::LoadFile("shaders/ShadeDeferred.frag.glsl"));
...
void RenderScene()
{
  GLFWwindow* window = Utility::CreateWindow({.name = "Deferred Example",
                                              .maximize = false,
                                              .decorate = true,
                                              .width = gWindowWidth,
  ...
  noiseTex.SubImage({.dimension = Fwog::UploadDimension::TWO,
                     .level = 0,
                     .offset = {},
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    shadingUniforms.sunViewProj = glm::ortho(-eyeWidth, eyeWidth, -eyeWidth, eyeWidth, .1f, 10.f) *
    ...
    Fwog::BeginRendering(gbufferRenderInfo);
    {
      ...
      Fwog::Cmd::DrawIndexed(static_cast<uint32_t>(gCubeIndices.size()),
                             static_cast<uint32_t>(objectUniforms.size()),
                             0,
                             0,
                             0);
    }
